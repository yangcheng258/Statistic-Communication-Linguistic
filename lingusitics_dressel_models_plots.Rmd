...
```{r}
ling <- read_csv("Study Responses (Final Fixed).csv") %>% 
  ...
  subset(Region!='Do not live in the United States' ) %>%
  subset(Age <= 30 )


ling$Region = factor(ling$Region, levels = c("Northeast", "Midwest", "Southeast"))
...
ling_mod <- ling  %>% 
  filter(Gender != "Non-Binary" & Region != "Midwest") %>% 
  mutate(Education = case_match(Education,
                                c("Currently in College", "Some High School, no Diploma", "High School") ~ "No degree",
                                c("Masters Degree", "Bachelors Degree") ~ "Degree")
  )

table1(~Ethnicity + Education + area_type + Gender + Age + Region, data = ling_mod)
```
...
ling_long_situation_percent %>%
  ...
  #           color = "white",
  #           fontface = "bold") +
  labs(title = "Frequency of dropping -g by situation",
       x = NULL,
       fill = NULL) +
...
ling_long_word_percent %>%
  ...
  #           color = "white",
  #           fontface = "bold") +
  labs(title = "Frequency of dropping -g by word",
       x = NULL,
       fill = NULL) +
   theme(axis.text.x = element_blank(),
        axis.title.x = element_blank(),
...
polr(as.factor(overall_in) ~ Region  + Age + Gender + Education  + area_type, data = ling_mod)

#best model just contains Region

```

```{r model for 2nd research question}
ling_sit_mod <- ling_long_situation  %>% 
  filter(Gender != "Non-Binary" & Region != "Midwest") %>% 
  mutate(Education = case_match(Education,
                                c("Currently in College", "Some High School, no Diploma", "High School") ~ "No degree",
                                c("Masters Degree", "Bachelors Degree") ~ "Degree")
...
situation <- clmm(as.factor(situation_in_usage) ~ Situation + Region + Gender + Age + (1|person_id), data = ling_sit_mod)
...
```{r}
situation <- clmm(as.factor(situation_in_usage) ~ Situation  + (1|person_id), data = ling_sit_mod)
...
clmm(as.factor(in_freq) ~ Word + (1|person_id), data = ling_word_mod) %>% 
  summary()
```
